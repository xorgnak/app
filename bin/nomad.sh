...
function commit() { git add . && git commit && git push; }
...
function arduino() {
source config.sh && arduino-cli compile --fqbn $FQBN "dev/$2" && arduino-cli upload --fqbn $FQBN --port $1 "dev/$2";
}
echo "arduino <port> <firmware>"
echo "nomad -> connect."
##### NOMADIC end #####
END
...
sudo chmod +x /usr/bin/leah

echo "$X DOMAINS: $DOMAINS"

echo "$X DEVS"

SSL="#nomad convention
server {
  listen 80;
  listen [::]:80;
...
echo "$X NGINX"
sudo cat << END > /etc/nginx/nginx.conf
user www-data;
...
END
echo "$X LOGO"
sudo cat << END > /etc/logo
####### NOMADIC LINUX ######
END

echo "$X ISSUE"
sudo cat << END > /etc/issue
`cat /etc/logo`
`uname -a`
